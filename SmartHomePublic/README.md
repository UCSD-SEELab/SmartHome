...
After finishing training, the function freeze_graph() stores the trained model for each device. The stored models can be recovered using export_mlp_weights.py. In export_mlp_weights.py, the function load_frozen_graph() loads the trained models back for each device and should be put in the device which needs to carry local computation. The function get_actual_weights() exports the weights of each trained model into txt files. In this way, we can convert the trained models into other languages such as C/C++ which is necessary for devices which do not have enough computation power. The function hierarchical_inference() gives an example on how to load the trained models back and does inference in the hierarchy. The basic idea is that each device sends the computed output to the next higher level devive. The higher level device concats all the output from the lower devices and use that as the input of its own model. For deployment, the code for concating the outputs of lower local devices should be placed in any devices which need to aggregate the lower level outputs. 