	...
	def segment(self):
		...
		for (start, end) in windowz(self.X, self.window_size, self.step):
		    if(len(self.X[start:end]) == self.window_size):
				...
				segments[i_segment] = np.concatenate(( skew_, max_, min_, mean, median_, stdev, var_, energy))
...
class Plugs(object):
	...
	def plot_voltage(self):
		self.toDataFrame()
		plt.plot_date(self.plug_df['TimeStamp'], self.plug_df['plug_1_voltage'])
		plt.plot_date(self.plug_df['TimeStamp'], self.plug_df['plug_2_voltage'])
		plt.plot_date(self.plug_df['TimeStamp'], self.plug_df['plug_3_voltage'])
		plt.xlabel("Time")
		plt.ylabel("Voltage")
		plt.show()
...
if __name__ == '__main__':
	...
	plug1, plug2, plug3 = RawData.get_plugs_data()
	...
	plug_label_df = plug_label_df.fillna(method="bfill")
	plug_label_df = plug_label_df.fillna(0)

	X = plug_label_df[['plug_1_current', 'plug_1_voltage', 'plug_2_current', 'plug_2_voltage', 'plug_3_current', 'plug_3_voltage']].astype('float').as_matrix()
