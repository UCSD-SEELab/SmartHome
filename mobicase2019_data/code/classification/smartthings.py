...
class Smartthings(object):
	...
	def toDataFrame(self):
		...
		range_ = pd.date_range(min(smartthings_df.TimeStamp), max(smartthings_df.TimeStamp), freq="S")
		df = pd.DataFrame(range_, columns=['TimeStamp'])
		smartthings_df = df.join(smartthings_df.set_index('TimeStamp'), on='TimeStamp', how='outer')
...
if __name__ == '__main__':
	...
	smartthings_df = smartthings_df.fillna(0)
	smartthings_df["smartthings/Basin Left Drawer/contact"] =  smartthings_df["smartthings/Basin Left Drawer/contact"].rolling(50).mean()
